...
pid_t alloc_process_id() {
  ...
  for (i = 0; i < PROCESS_MAX_PROCESSES; ++i) {
    if (process_table[i].pid == -1) {
      ...
      process_table[i].state = PROCESS_WORKING;
      break;
    }
  }
                      ...
                      virtaddr_t *entry_point, virtaddr_t *stack_top)
{
  ...
  /* Make sure the file existed and was a valid ELF file */ if (file < 0) {
    return -1;
  }
...
//TEST FUNKTION 
/*
void process_is_done(){
  for(int i = 0;i < PROCESS_MAX_PROCESSES; i++){
    if(process_table[i].state == PROCESS_DONE){
      kprintf("WE HAVE A NOT DONE PROCESS");
    }
    else kprintf("ALL DONE");
  }
} */
